...
class VLMFrontendView(ctk.CTkFrame):
    ...
    def toggle_vlm_server(self):
        ...
        threading.Thread(target=self._start_server_bg).start()
    ...
    def _start_server_bg(self):
        try:
            ...
            logging.info(
                "[VLM] Launch config: ngl=%s ctx=%s threads=%s batch=%s n_predict=%s",
                config["n_gpu_layers"],
                config["ctx_size"],
                config["threads"],
                config["batch_size"],
    ...
    def send_analysis(self, event=None):
        ...
        threading.Thread(target=self._run_vision_inference, args=(self.current_image_path, prompt), daemon=True).start()

    def _run_vision_inference(self, image_path, prompt):
        try:
            logging.info(f"[VLM] Starting vision inference on {image_path}")
    ...
    def refresh_models(self):
        ...
        # Look for GGUF files (VLM models)
